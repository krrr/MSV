...
namespace DirectInput {
    struct ClassServiceData : public CONNECT_DATA {
        ...
        NTSTATUS Initialize(Classes aClass);

        NTSTATUS CallService(PVOID aInputData, PVOID aInputDataEnd, UINT32* aConsumed);
    };
    ...
    NTSTATUS ClassServiceData::Initialize(Classes aClass) {
#   pragma prefast(push)
...
#   pragma prefast(pop)
    ...
    NTSTATUS ClassServiceData::CallService(PVOID aInputData, PVOID aInputDataEnd, UINT32* aConsumed) {
        struct DPCContext {
            KDPC            Dpc;
            KEVENT          Event;
            PVOID           Callback;
    ...
    template<typename T>
    static NTSTATUS MouseInput($MOUSEINPUT<T>& aInput) {
        NTSTATUS vStatus    = STATUS_SUCCESS;
    ...
    template<typename T>
    static NTSTATUS KeyboardInput($KEYBDINPUT<T>& aInput) {
        NTSTATUS vStatus    = STATUS_SUCCESS;
    ...
    NTSTATUS SendInput(UINT32 aInputCount, LPINPUT aInputs, UINT32 aInputBytes, UINT32* aConsumed) {
        NTSTATUS vStatus = STATUS_SUCCESS;
